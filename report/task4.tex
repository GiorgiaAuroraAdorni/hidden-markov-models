...
\Solution{
	\begin{itemize}
		\item[(a)] In the leave-one-out cross-validation, $n$ models are 
		trained on $n-1$ data-points and validated on the other. First of all, 
		the train set losses are computed:
		...
		Since we want to find the value of $c$ that minimises the squared 
		error, it is necessary to compute the derivatives of the losses $l$ 
		with respect to $c$ and finding the value of $c$ that satisfies $l=0$:
